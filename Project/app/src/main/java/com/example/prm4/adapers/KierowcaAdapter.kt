    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): KierowcaViewHolder {
        ...
        return KierowcaViewHolder(binding).also { vh ->
            binding.root.setOnClickListener{
                if(data[vh.absoluteAdapterPosition].icon == "mercedes")
                    DishImagesAdapter.set(0)
                if(data[vh.absoluteAdapterPosition].icon == "red_bulll")
                    DishImagesAdapter.set(1)
                if(data[vh.absoluteAdapterPosition].icon == "ferrari")
                    DishImagesAdapter.set(2)
